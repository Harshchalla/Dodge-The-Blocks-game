...
void restart()// to restart the game
{
    ...
    if(score>=highscore)
    {
        glColor3f(0.9,0.7,0);
        highscore = score;
        drawText("Congrats Its A New HighScore", 190,310);
        drawText("Press Space To Restart", 190, 160);
    }
    else{
    
    drawText("Points Away From HighScore :", 190,310);
    sprintf( buffer, "%d", (highscore-score) );
    drawTextNum(buffer, 6, 350,310);
    
    glColor3f(0,1,.5);
    drawText("Try Your Luck Next Time", 190, 180);
        drawText("Press Space To Restart", 190, 160);
    }
    glutSwapBuffers();
...
void display()
{
    if(screen == 0)
    {
        ...
        glColor3f(1,1,1);
        drawText("BY:1DT19CS054 AND 1DT19CS051",300,25);
        glutSwapBuffers();
    }
...
int main(int argc, char **argv)
{
    ...
    glutCreateWindow("2D Dodge Blocks game");
    ovpos();
    init();
